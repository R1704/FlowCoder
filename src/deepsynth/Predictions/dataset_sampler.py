import random
import torch
from deepsynth.type_system import STRING, Type, List, INT, BOOL
from deepsynth.cons_list import tuple2constlist

from deepsynth.flashfill_dataset_loader import randomWord
...
class Dataset(torch.utils.data.IterableDataset):
    ...
    def __single_data__(self):
        ...
        while flag or output == None:
            ...
            inputs = [[self.input_sampler.sample(type_) for type_ in self.arguments[selected_type]] for _ in
                      range(nb_IOs)]
...
class Input_sampler():
    ...
    However we sample elements from a lexicon so we cannot sample both a int list and a string for example since in that case the lexicon would need to conatin integers and strings.
