...
class Data:
    ...
    def __init__(self, max_program_depth=6, shuffle=False):
        ...
        self.task_generator = self.make_dreamcoder_task_generator(self.tasks)
    ...
    def get_next_batch(self, batch_size):
        ...
        task = next(self.task_generator)  # NOTE: This should be moved two rows down if you want different tasks for every batch
        batch_program_names = []
    ...
    def make_dreamcoder_task_generator(self, tasks):
        while True:
            if self.shuffle:
    ...
    # Resets the task generator to its initial state.
    def reset_task_generator(self):
        self.task_generator = self.make_dreamcoder_task_generator(self.tasks)
