from src.sequential.deepsynth.cons_list import cons_list2list
import typing
import ray
...
import logging
from src.sequential.deepsynth.program import BasicPrimitive, Function, New, Program, Variable
import time
from typing import Callable, Tuple
import src.sequential.deepsynth.grammar_splitter as grammar_splitter
from src.sequential.deepsynth.Algorithms.ray_parallel import start, make_parallel_pipelines

from src.sequential.deepsynth.Algorithms.heap_search import heap_search
from src.sequential.deepsynth.Algorithms.heap_search_naive import heap_search_naive
from src.sequential.deepsynth.Algorithms.a_star import a_star
from src.sequential.deepsynth.Algorithms.threshold_search import threshold_search
from src.sequential.deepsynth.Algorithms.dfs import dfs
from src.sequential.deepsynth.Algorithms.bfs import bfs
from src.sequential.deepsynth.Algorithms.sort_and_add import sort_and_add
from src.sequential.deepsynth.Algorithms.sqrt_sampling import sqrt_sampling, sqrt_sampling_with_sbsur

from src.sequential.deepsynth.program_as_list import reconstruct_from_compressed
...
def run_algorithm(is_correct_program: Callable[[Program, bool], bool], pcfg: PCFG, algo_index: int) -> Tuple[
    Program, float, float, int, float, float]:
    '''
...
def run_algorithm_parallel(is_correct_program: Callable[[Program, bool], bool], pcfg: PCFG, algo_index: int,
                           splits: int,
                           n_filters: int = 4, transfer_queue_size: int = 500_000, transfer_batch_size: int = 10) -> \
Tuple[Program, float, typing.List[float], typing.List[float], typing.List[int], typing.List[float], float]:
    '''
    Run the algorithm until either timeout or 1M programs, and for each program record probability and time of output
    return program, search_time, evaluation_time, nb_programs, cumulative_probability, probability
    '''
    ...
    logging.debug(
        "\tStats: found={} generated programs={} evaluated programs={} covered={:.1f}%".format(found, sum(nb_programs),
...
def gather_data(dataset: typing.List[Tuple[str, PCFG, Callable]], algo_index: int) -> typing.List[
    Tuple[str, Tuple[Program, float, float, int, float, float]]]:
...
def gather_data_parallel(dataset: typing.List[Tuple[str, PCFG, Callable]], algo_index: int, splits: int,
                         n_filters: int = 4, transfer_queue_size: int = 500_000, transfer_batch_size: int = 10) -> \
typing.List[Tuple[
    str, Tuple[Program, float, typing.List[float], typing.List[float], typing.List[int], typing.List[float], float]]]:
    algorithm, _, _ = list_algorithms[algo_index]
