...
# for fast weighted sampling
import vose

from src.sequential.deepsynth.type_system import Type, PolymorphicType, PrimitiveType, Arrow, List, UnknownType, INT, BOOL
from src.sequential.deepsynth.program import Program, Function, Variable, BasicPrimitive, New
...
class PCFG:
    ...
    def get_sbsur_sampler(self, S=None, seed=None):
        ...
        def seq2prog(sequence):
            ...
            while context_stack:
                current = context_stack.pop()
                assert len(self.list_derivations[
                               current]) == 1, f"Current: {current} has more than 1 derivation:{self.list_derivations[current]}"
                P = self.list_derivations[current][0]
                args_P, w = self.rules[current][P]
                program = (P, program)
                for arg in args_P:
